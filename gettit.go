...
func Printprogress(path string, total float64) {
	...
	for i := 0; i < 20-int(barpercent)/5; i++ {
		bar = fmt.Sprint(bar, " ") //And fill the rest with spaces, we want the bar to be 20 wide
		//(excluding the brackets) so we fill the difference between the number of bars we will fill and 20
	}
	bar = fmt.Sprint(bar, "]")
...
func DLfile(url string, saveas string, size int64) {
	...
	case "gif":
		if outfile == "placeholder" {
			outfile = Generaterandomstring() + ".gif"
			//If you want to use the -o flag you should also specify the extension, so if you use it file
			//extensions won't be automatically handled (or else we would end up wiht file.extension.extension)
		}
		filename = outfile //Since we only have to download the gif itself and no audio, we don't need
		//temporary files, so we can "directly" name the downloaded file as the output file specified in args.
...
func main() {
	...
	req.Header.Set("user-agent", "Mozilla/5.0 (X11; Linux x86_64; rv:107.0) Gecko/20100101 Firefox/107.0")
	//We need a semi-credible user agent or else reddit will just block the request
	...
	case "video":
		if outfile == "placeholder" {
			outfile = Generaterandomstring() + ".mp4" //If you want to use the -o flag you should also
			//pecify the extension, so if you use it file extensions won't be automatically handled
			//(or else we would end up wiht file.extension.extension)
		}
